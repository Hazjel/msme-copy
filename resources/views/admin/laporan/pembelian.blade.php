    ...
    <div class="card">
        ...
        <div class="card-body table-responsive p-0">
            <table class="table table-bordered">
                ...
                <tbody>
                    @forelse ($pembelians as $pembelian)
                        @foreach ($pembelian->details as $idx => $detail)
                            <tr>
                                @if ($idx === 0)
                                    <td rowspan="{{ $pembelian->details->count() }}">
                                        {{ $pembelian->tanggal->format('d-m-Y') }}
                                    </td>
                                    <td rowspan="{{ $pembelian->details->count() }}">{{ $pembelian->nomor }}</td>
                                    <td rowspan="{{ $pembelian->details->count() }}">{{ $pembelian->supplier->nama }}
                                    </td>
                                @endif
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center text-muted py-3">Tidak ada data pembelian pada
                ...
                <tfoot>
                    <tr>
                        <th colspan="6" class="text-right">Total Pembelian</th>
